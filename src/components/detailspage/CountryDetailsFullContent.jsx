          ...
          {country.borders?.length > 0 ? (
            country.borders.map((border) => (
              <Link key={border} to={`/${filterString(getBorderCountryName(border, countries))}`}>
                <li className="border border-transparent shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-md p-2 m-1 hover:border-slate-300 transition ease-out delay-150">
                  {getBorderCountryName(border, countries)}
                </li>
              </Link>
            ))
          ) : (
