...
export default function Home() {
  ...
  return (
    ...
    >
      <div className="min-w-full flex justify-between align-center flex-wrap w-full">
      ...
      {!loading && !error && (
        <section className={`min-w-full gap-4 grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 justify-items-center w-full`}>
          {displayContent.map((country) => (
            ...
            <Suspense fallback={<Loading />} key={country.name + country.capital}>
              <Link to={filterString(country.name)}>
                <CountryCard
